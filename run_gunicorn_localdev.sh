...
#gunicorn -w 2 -b 0.0.0.0:8080 --log-config=gunicorn-logging.conf myflaskpackage.flask_module:app

# python file for app settings
# conf file for log settings
gunicorn --config gunicorn.conf.py --log-config=gunicorn-logging.conf maintmodule.app:app
